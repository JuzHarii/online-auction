...
function ProductsTab() {  
  ...
  return(
    <div className="flex flex-col gap-5">
      <p className="text-gray-500">You posted {products.length} product{products.length > 1 ? "s" : ""}</p>
        ...
        return (
          <div 
            ...
            <div className="flex flex-col gap-5 flex-grow">
              <div className="flex flex-row gap-5 justify-between">
                <div>
                  <Link to={`/product/${product.product_id}`} className="hover:text-[#8D0000] cursor-pointer text-2xl font-bold">{product.product_name}</Link>
                  <div className="text-md text-gray-400">{product.category_name}</div>
                </div>
                <div className="flex flex-col place-items-end">
                  <button 
              ...
              <div className="flex flex-row gap-5">
                ...
                <div className="flex-2 min-w-0">
                  <label className="font-medium text-lg">Highest bidder</label>
                  <div className="font-medium text-xl text-[#8D0000] mb-2">{product.highest_bidder_name}</div>
                </div>
                <div className="flex-1 min-w-0">
                  <label className="font-medium">Buy now price:</label>
                  <div>{formatCurrency(product.buy_now_price?.toString())}</div>
      ...
      <div className="flex flex-row">
        {
          tabs.map((tab) => {
            ...
            if (profile.role === "bidder" && tab === "My products")
              return(<></>)
